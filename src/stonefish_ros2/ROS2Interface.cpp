...
std::vector<uint8_t> serializePointCloud(const PointCloud& cloud) {
    ...
    buffer.insert(buffer.end(), reinterpret_cast<const uint8_t*>(&cloud.timestamp), 
                  reinterpret_cast<const uint8_t*>(&cloud.timestamp) + sizeof(cloud.timestamp));

    // Serialize the frame_id string
    uint32_t frame_id_size = cloud.frame_id.size();
    buffer.insert(buffer.end(), reinterpret_cast<uint8_t*>(&frame_id_size), 
                  reinterpret_cast<uint8_t*>(&frame_id_size) + sizeof(frame_id_size));
    buffer.insert(buffer.end(), cloud.frame_id.begin(), cloud.frame_id.end());

    // Serialize the number of points in the cloud
    uint32_t num_points = cloud.points.size();
    buffer.insert(buffer.end(), reinterpret_cast<uint8_t*>(&num_points), 
                  reinterpret_cast<uint8_t*>(&num_points) + sizeof(num_points));

    // Serialize each point in the cloud
    for (const auto& point : cloud.points) {
        // Serialize x
        buffer.insert(buffer.end(), reinterpret_cast<const uint8_t*>(&point.x), 
                      reinterpret_cast<const uint8_t*>(&point.x) + sizeof(point.x));
        
        // Serialize y
        buffer.insert(buffer.end(), reinterpret_cast<const uint8_t*>(&point.y), 
                      reinterpret_cast<const uint8_t*>(&point.y) + sizeof(point.y));
        
        // Serialize z
        buffer.insert(buffer.end(), reinterpret_cast<const uint8_t*>(&point.z), 
                      reinterpret_cast<const uint8_t*>(&point.z) + sizeof(point.z));
        
        // Serialize intensity
        buffer.insert(buffer.end(), reinterpret_cast<const uint8_t*>(&point.intensity), 
                      reinterpret_cast<const uint8_t*>(&point.intensity) + sizeof(point.intensity));
    }
...
void ROS2Interface::PublishEventBasedCamera(const std::shared_ptr<rclcpp::Node> nh, rclcpp::PublisherBase::SharedPtr pub, EventBasedCamera* ebc)
{
    ...
    for(size_t i=0; i<msg.events.size(); ++i)
    {
        ...
        // Add the rclcpp::Duration to the rclcpp::Time
        msg.events[i].ts = header_time + rclcpp::Duration(0, std::abs(data[i*2+1]));
