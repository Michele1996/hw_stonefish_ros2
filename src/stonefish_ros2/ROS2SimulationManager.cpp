...
void ROS2SimulationManager::BuildScenario()
{
    ...
    srvs_["enable_light"] = nh_->create_service<std_srvs::srv::Trigger>("enable_light", std::bind(&ROS2SimulationManager::EnableCurrentsService, this, _1, _2));
    srvs_["disable_light"] = nh_->create_service<std_srvs::srv::Trigger>("disable_light", std::bind(&ROS2SimulationManager::DisableCurrentsService, this, _1, _2));
    srvs_["enable_thruster"] = nh_->create_service<std_srvs::srv::Trigger>("enable_thruster", std::bind(&ROS2SimulationManager::EnableCurrentsService, this, _1, _2));
    srvs_["disable_thruster"] = nh_->create_service<std_srvs::srv::Trigger>("disable_thruster", std::bind(&ROS2SimulationManager::DisableCurrentsService, this, _1, _2));
    ...
    simulation_info_publisher_ = nh_->create_publisher<stonefish_ros2::msg::SimulationInfo>("simulation_info_topic", 10);
...
void ROS2SimulationManager::SimulationStepCompleted(Scalar timeStep)
{
            ...
            switch(actuator->getType())
            {
                ...
                case ActuatorType::THRUSTER:
                {
                    ...
                    if(it != pubs_.end())
                    {
                        ...
                        msg.position.push_back(th->getAngle()/(Scalar(2)*M_PI)*Scalar(60));
                        msg.velocity.push_back(th->getOmega()/(Scalar(2)*M_PI)*Scalar(60));
    ...
    stonefish_ros2::msg::SimulationInfo sim_info_message = stonefish_ros2::msg::SimulationInfo();
...
void ROS2SimulationManager::SegmentationCameraImageReady(SegmentationCamera* cam)
{
    ...
    //Fill in the image message
    sensor_msgs::msg::Image::SharedPtr img = std::get<0>(dualImageCameraMsgPrototypes[cam->getName()]);
    img->header.stamp = nh_->get_clock()->now();
    memcpy(img->data.data(), (float*)cam->getImageDataPointer(), img->step * img->height);

    //Fill in the info message
    sensor_msgs::msg::CameraInfo::SharedPtr info = std::get<1>(dualImageCameraMsgPrototypes[cam->getName()]);
    info->header.stamp = img->header.stamp;

    //Fill in the display message
    sensor_msgs::msg::Image::SharedPtr img2 = std::get<2>(dualImageCameraMsgPrototypes[cam->getName()]);
...
{
    ROS2Interface::PublishEventBasedCamera(nh_, pubs_.at(cam->getName()), cam);
}
...
void ROS2SimulationManager::EnableThrusterService(const std_srvs::srv::Trigger::Request::SharedPtr req, 
                                           std_srvs::srv::Trigger::Response::SharedPtr res)
{
    (void)req;
    for(size_t i=0; i<rosRobots_.size(); ++i){
        if(rosRobots_[i]->robot_->getName()==""){
          for(size_t j=0; j<rosRobots_[i]->robot_->getActuators().size();j++){
              if(rosRobots_[i]->robot_->getActuator(j)->getType()==ActuatorType::THRUSTER && rosRobots_[i]->robot_->getActuator(j)->getName()==""){
                 rosRobots_[i]->robot_->getActuator(j)->setActive(true);
              }
        }   
    }
    res->message = "Actuator disabled.";
    res->success = true;
}
}

void ROS2SimulationManager::DisableThrusterService(const std_srvs::srv::Trigger::Request::SharedPtr req, 
                                            std_srvs::srv::Trigger::Response::SharedPtr res)
{   
    (void)req;
    for(size_t i=0; i<rosRobots_.size(); ++i){
        if(rosRobots_[i]->robot_->getName()==""){
          for(size_t j=0; j<rosRobots_[i]->robot_->getActuators().size();j++){
              if(rosRobots_[i]->robot_->getActuator(j)->getType()==ActuatorType::THRUSTER && rosRobots_[i]->robot_->getActuator(j)->getName()==""){
                 rosRobots_[i]->robot_->getActuator(j)->setActive(false);
              }
        }   
    }
    res->message = "Actuator disabled.";
...
void ROS2SimulationManager::EnableLightService(const std_srvs::srv::Trigger::Request::SharedPtr req, 
                                           std_srvs::srv::Trigger::Response::SharedPtr res)
{
    (void)req;
    for(size_t i=0; i<rosRobots_.size(); ++i){
        if(rosRobots_[i]->robot_->getName()==""){
          for(size_t j=0; j<rosRobots_[i]->robot_->getActuators().size();j++){
              if(rosRobots_[i]->robot_->getActuator(j)->getType()==ActuatorType::LIGHT && rosRobots_[i]->robot_->getActuator(j)->getName()==""){
                 ((Light*)rosRobots_[i]->robot_->getActuator(j))->getGLLight()->SwitchOn();
              }
        }   
    }
    res->message = "Light disabled.";
    res->success = true;
}
}

void ROS2SimulationManager::DisableLightService(const std_srvs::srv::Trigger::Request::SharedPtr req, 
                                            std_srvs::srv::Trigger::Response::SharedPtr res)
{   
    (void)req;
    for(size_t i=0; i<rosRobots_.size(); ++i){
        if(rosRobots_[i]->robot_->getName()==""){
          for(size_t j=0; j<rosRobots_[i]->robot_->getActuators().size();j++){
              if(rosRobots_[i]->robot_->getActuator(j)->getType()==ActuatorType::LIGHT && rosRobots_[i]->robot_->getActuator(j)->getName()==""){
                 ((Light*)rosRobots_[i]->robot_->getActuator(j))->getGLLight()->SwitchOff();
              }
        }   
    }
    res->message = "Light disabled.";
...
void ROS2SimulationManager::ServosCallback(const sensor_msgs::msg::JointState::SharedPtr msg, std::shared_ptr<ROS2Robot> robot)
{
    ...
    else if(msg->velocity.size() > 0)
    {
        for(size_t i=0; i<msg->velocity.size(); ++i)
        {
            ...
            {
                robot->servoSetpoints_.at(msg->name[i]) = std::make_pair(ServoControlMode::VELOCITY, (Scalar)msg->velocity[i]);
                RCLCPP_WARN_STREAM(nh_->get_logger(), "CHECK" << msg->name[i]);
            }
            catch(const std::out_of_range& e)
            {
                RCLCPP_WARN_STREAM(nh_->get_logger(), "Invalid joint name in desired joint state message: " << msg->name[i]);
